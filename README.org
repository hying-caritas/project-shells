...
The package could be setup via call
src_emacs-lisp{project-shells-setup}, the parameter is the keymap to
add project shells key binding.  For example, to use project shells
with projectile you can put the following forms in your init file,
...
Use src_emacs-lisp{customize-group} project-shell to change the
configuration.
...
src_emacs-lisp{project-shells-project-name-func} and
src_emacs-lisp{project-shells-project-root-func}.
...
src_emacs-lisp{project-shells-histfile-env} may need to be customized.
...
src_emacs-lisp{project-shells-default-init-file} may need to be
customized.
